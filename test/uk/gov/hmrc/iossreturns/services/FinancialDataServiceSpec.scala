...
class FinancialDataServiceSpec extends SpecBase with ScalaCheckPropertyChecks
  ...
  "Financial Data Service" - {
    ...
    "return back the exact result retrieved from the connector" in {
      ...
      whenReady(financialDataService.getFinancialData(FinancialDataConnectorFixture.vrn, None, None), PatienceConfiguration.Timeout(Span(2, Seconds))) { maybeFinancialData =>
