...
class GetRegistrationAction @Inject()(
  ...
  override protected def refine[A](request: IdentifierRequest[A]): Future[Either[Result, RegistrationRequest[A]]] = {
    ...
    registrationConnector.get()(hc).map { etmpRegistration =>
      Right(RegistrationRequest(request.request, request.credentials, request.vrn, request.iossNumber, etmpRegistration))
    }
  }
}